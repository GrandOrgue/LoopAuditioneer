...
{	static SF_INSTRUMENT write_inst =
	...
	SF_CUES read_cues ; /* added to check for correct handling of aiff INSTR chunk */
	SNDFILE	 *file ;
	SF_INFO	 sfinfo ;
	...
	memset (&read_cues, 0, sizeof (read_cues)) ; /* Added to check correct aiff loop handling */
	...
	{ /* If it's an aiff file then we need the MARK (cue) chunk for the loops in INST */
		if (sf_command (file, SFC_GET_CUE, &read_cues, sizeof (read_cues)) == SF_FALSE)
		{	printf ("\n\nLine %d : sf_command (SFC_GET_CUES) failed.\n\n", __LINE__) ;
			exit (1) ;
			return ;
		} ;
	}
	...
	if ((filetype & SF_FORMAT_TYPEMASK) == SF_FORMAT_AIFF)
	{	/*
		**	For the loop fields aiff files should NOT hold the position
		**	but rather a unique MarkerID that references a marker which holds
		**	the actual position.
		*/
		int idx, refSustainLoopStart, refSustainLoopEnd, refReleaseLoopStart, refReleaseLoopEnd;
		
		refSustainLoopStart = read_inst.loops[0].start;
		refSustainLoopEnd = read_inst.loops[0].end;
		refReleaseLoopStart = read_inst.loops[1].start;
		refReleaseLoopEnd = read_inst.loops[1].end;
		for (idx = 0; idx < read_cues.cue_count; idx++) {
			if (read_cues.cue_points[idx].dwName == refSustainLoopStart)
				read_inst.loops[0].start = read_cues.cue_points[idx].dwPosition;
			if (read_cues.cue_points[idx].dwName == refSustainLoopEnd)
				read_inst.loops[0].end = read_cues.cue_points[idx].dwPosition;
			if (read_cues.cue_points[idx].dwName == refReleaseLoopStart)
				read_inst.loops[1].start = read_cues.cue_points[idx].dwPosition;
			if (read_cues.cue_points[idx].dwName == refReleaseLoopEnd)
				read_inst.loops[1].end = read_cues.cue_points[idx].dwPosition;
		}
	} ;
