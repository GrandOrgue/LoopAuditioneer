	...
	{	psf_binheader_readf (psf, "m", &marker) ;
		...
		{	case RIFF_MARKER :
			...
			case cue_MARKER :
					...
					{	unsigned bytesread, position, offset ;
						int cue_count, id, chunk_id, chunk_start, block_start ;
						int cue_index ;
						...
						if ((psf->cues = psf_cues_alloc ()) == NULL) /* added by Lars Palo */
							return SFE_MALLOC_FAILED ;

						psf->cues->cue_count = cue_count ; /* added by Lars Palo */
...
{	unsigned int bytesread = 0, dword, sampler_data, loop_count ;
	unsigned int note, start, end, type = -1, count, pitch_fraction ;
	int j, k ;
	...
	bytesread += psf_binheader_readf (psf, "4", &dword) ;
	pitch_fraction = dword; /* Lars Palo */
	if (dword != 0)
	...
	*/
	...
	psf->instrument->dwMIDIPitchFraction = pitch_fraction ; /* Lars Palo */
