	...
	{	psf_binheader_readf (psf, "m", &marker) ;
		...
		{	case FORM_MARKER :
			...
			case MARK_MARKER :
					...
					pchk4_store (&paiff->chunk4, marker, psf_ftell (psf) - 8, dword) ;
					psf_log_printf (psf, " %M : %d\n", marker, dword) ;
					{	unsigned short mark_id, n = 0, pstringIdx ;
						unsigned int position ;
						...
						{	psf_log_printf (psf, "  More than 100 markers, skipping!\n") ;
							psf_binheader_readf (psf, "j", dword - 2) ;
							break ;
							} ;
						...
						{	unsigned int pstr_len ;
							...
							psf->cues->cue_points[n].dwPosition = position ;
							...
							for (pstringIdx = 0 ; pstringIdx < sizeof (psf->u.scbuf); pstringIdx++ ) 
								psf->cues->cue_points[n].name[pstringIdx] = psf->u.scbuf[pstringIdx];
...
{	sf_count_t		current ;
	...
	/* Check to see if there's a MARK chunk (cues) to write */
	if (psf->cues != NULL)
	{	int totalStringLength = 0, idx, ps, stringLength;
		...
		psf_binheader_writef (psf, "Em4", MARK_MARKER, 2 + psf->cues->cue_count * (2 + 4) + totalStringLength) ;
		psf_binheader_writef (psf, "E2", (short) psf->cues->cue_count) ;

		for (idx = 0; idx < psf->cues->cue_count ; idx++) 
		{	psf_binheader_writef (psf, "E24", (short) psf->cues->cue_points[idx].dwName,  psf->cues->cue_points[idx].dwPosition) ;
	...
	{	/* There shouldn't be any INST chunk if there's no MARK (cues) so this is for the correct use */

		/* First convert loop modes to aiff standard */
		short int typeMode, sustainLoopMode, releaseLoopMode;
		...
		psf_binheader_writef (psf, "E2222222", (short) psf->instrument->gain, 
			sustainLoopMode, (unsigned short) psf->instrument->loops[0].start, (unsigned short) psf->instrument->loops[0].end, 
			releaseLoopMode, (unsigned short) psf->instrument->loops[1].start, (unsigned short) psf->instrument->loops[1].end) ;
	
	} else if (psf->instrument != NULL && psf->cues == NULL) 
	{	/* This should really not happen, but is included here for backward compatibility with earlier behaviour */
		/* We'll expect that the loop start and end will contain the position information and not references to markers */
		short int typeMode, sustainLoopMode, releaseLoopMode;
		unsigned int sLoopStart = 0, sLoopEnd = 0, rLoopStart = 0, rLoopEnd = 0;
