...
{	sf_count_t		current ;
	...
	if (psf->cues != NULL && psf->instrument == NULL)
	{	/* This is a use case where no loops exist but there are cues */
		...
		for (idx = 0; idx < psf->cues->cue_count ; idx++) 
		{	psf_binheader_writef (psf, "E24", (short) psf->cues->cue_points[idx].dwName,  psf->cues->cue_points[idx].dwSampleOffset) ;
